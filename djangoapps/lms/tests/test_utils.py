...
class TestUtils(TestCase):
    ...
    def setUp(self):
        ...
        for number in range(10):
            lesson = self._add(Lesson.objects.create(course=self.course, title=f'lesson {number}', ))
        ...
        self.flow = self._add(Flow.objects.create(course=self.course, name="flow 1"))
    ...
    def test_lessons_open_ready(self):
        ...
        assert not self.flow.flow_lessons.get(number=2).is_opened
        assert not self.flow.flow_lessons.get(number=1).is_opened
        # let's set the first flow_lesson as to be open at past
        flow_lesson: FlowLesson = self.flow.flow_lessons.get(number=1)
        today = now()
        ...
        assert not self.flow.flow_lessons.get(number=1).is_opened, "For flow which is not started doesn't open the lessons"
        assert not self.flow.flow_lessons.get(number=2).is_opened, "Future lessons should not open"

        self.flow.state = Flow.STATE_ONGOING
        self.flow.save()
        lessons_open_ready()
        flow_lesson: FlowLesson = self.flow.flow_lessons.first()
        assert self.flow.flow_lessons.get(number=1).is_opened, "For flow which is started should open the lessons"
        assert not self.flow.flow_lessons.get(number=2).is_opened, "Future lessons should not open"
